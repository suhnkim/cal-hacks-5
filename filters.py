...
class Filter():
    ...
    def __init__(self, type):
        if type =="time":
            self.perform_filter = self.time_filter
        elif type == "department":
            self.deparments = DEPARTMENTS
            self.perform_filter = self.department_filter
        elif type == "classes":
            self.classes = CLASSES
            self.perform_filter = self.classes_filter
        elif type == "research":
            self.research_topics = RESEARCH_TOPICS
            self.perform_filter = self.research_filter
        elif type == "interests":
            self.perform_filter = self.interest_filter
